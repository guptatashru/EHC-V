...
app.controller('patientSearchCtrl', function ($scope, $http, $location, $filter, $state, Idle, Keepalive, $rootScope, sqlCypherDb, $localStorage, urlHitService, dialogs, $stateParams, windowsFileDownload, $q) {
    //$scope.date = new Date();
    ...
    $scope.fileManager = {
        ...
        receivedEvent: function (url, fileName, location) {
                        ...
                        for(var i in result.data.syncObj.AppointmentSyncObjects)
                        {
                            ...
                            try {
                                ...
                                for (var j in tempPatientDetails.documents){
                                    $scope.fileManager.receivedEvent(tempPatientDetails.documents[j].documentURL, tempPatientDetails.documents[j].documentName, "id_" + tempPatientDetails.patient.id);
                                    if (device.platform === "windows") {
                                        tempPatientDetails.documents[i].documentURL = "ms-appdata:///local/pdfs/id_" + tempPatientDetails.patient.id + "/" + tempPatientDetails.documents[j].documentName;
                                    }
                                    else {
...
app.controller('selectPatientCtrl', ['$scope', '$http', '$element', '$rootScope', '$state', '$localStorage', 'dialogs', '$stateParams', 'sqlCypherDb', '$filter', function ($scope, $http, $element, $rootScope, $state, $localStorage, dialogs, $stateParams, sqlCypherDb, $filter) {
    ...
    $scope.onPatientClicker = function (patientClicker) {
        ...
        $scope.patientDataLoadedSuccessFunction = function (data, status, headers, config) {
            if (data["patient"]) {
                $scope.patientSelected.patient = data["patient"];
                $scope.patientInfoSet = true;
            }
            else if (data["contact"]) {
                $scope.patientSelected.contact = data["contact"];
                $scope.contactInfoSet = true;
            }
            else if (data["choice"]) {
                $scope.patientSelected.choice = data["choice"];
                $scope.choiceInfoSet = true;
            }
            else if (data["employer"]) {
                $scope.patientSelected.employer = data["employer"];
                $scope.employerInfoSet = true;
            }
            else if (data["misc"]) {
                $scope.patientSelected.misc = data["misc"];
                $scope.miscInfoSet = true;
            }
            else if (data["notes"]) {
                $scope.patientSelected.notes = data["notes"];
                $scope.notesInfoSet = true;
            }

            if ($scope.patientInfoSet == true && $scope.contactInfoSet == true && $scope.choiceInfoSet == true && $scope.employerInfoSet == true && $scope.miscInfoSet == true && $scope.notesInfoSet == true) {
                //console.log($scope.patientSelected);
                $state.go('patientDetails', {
                    patientListData: {
                        tabSelection: $scope.filterType,
                        data: $scope.patients
                    },
                    patientDetailsObject: { patientID : -1}
                });
            }
        }

        $scope.patientDataLoadingError = function (data, status, headers, config) {
            console.log("data load error");
        }
        //$state.go('patientDetails', { patientDetailsObject: patientClicker });

        /*$http.get($rootScope.urls.patientDetailsUrl + patientClicker.patientID).
            success($scope.patientDataLoadedSuccessFunction).
            error($scope.patientDataLoadingError);
        $http.get($rootScope.urls.patientContactsUrl + patientClicker.patientID).
            success($scope.patientDataLoadedSuccessFunction).
            error($scope.patientDataLoadingError);
        $http.get($rootScope.urls.patientChoicesUrl + patientClicker.patientID).
            success($scope.patientDataLoadedSuccessFunction).
            error($scope.patientDataLoadingError);
        $http.get($rootScope.urls.patientEmployerUrl + patientClicker.patientID).
            success($scope.patientDataLoadedSuccessFunction).
            error($scope.patientDataLoadingError);
        $http.get($rootScope.urls.patientMiscUrl + patientClicker.patientID).
            success($scope.patientDataLoadedSuccessFunction).
            error($scope.patientDataLoadingError);
        $http.get($rootScope.urls.patientNotesUrl + patientClicker.patientID).
           success($scope.patientDataLoadedSuccessFunction).
           error($scope.patientDataLoadingError);*/
...
app.controller('patientDetailsCtrl', function ($scope, $http, $location, $state, Idle, Keepalive, $stateParams, $sce, $localStorage, dialogs, $rootScope) {
    ...
    $scope.onPatientRemindersTabClicked = function (reminder) {
        ...
        if (reminder == "clinical") {
            ...
            $scope.remindersSelectedHeader.path = "templates/directives/patientDetailsPage/remindersTabContents/TabContent.html";
