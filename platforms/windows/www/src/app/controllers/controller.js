...
app.controller('pdfPageCtrl', function ($scope, $http, Idle, Keepalive, $state, $stateParams) {  //controller for the state 'pdfViewerPage'
    $scope.hardwareBackButtonClicked = function () {                                                //defining a function on back button click
       
        $state.go('patientDetails', { patientListData: $stateParams.patientListData, patientDetailsObject: null });     //navigates to state mentioned 
    }
...
app.controller('pdfViewerCtrl', ['$scope', '$sce', '$http', '$location', '$state', 'PDFViewerService', '$stateParams', '$rootScope', function ($scope, $sce, $http, $location, $state, pdf, $stateParams, $rootScope) {
    ...
    $scope.pdfDocumentName = $stateParams.document.documentName;        //name of the document selected

    $scope.pdfURL = $sce.trustAsResourceUrl($stateParams.document.documentURL);     //url of the selected document
...
app.controller('loginMenuCtrl', ['$scope', '$http', '$state', '$rootScope', '$localStorage', 'sqlCypherDb', '$interval','$location','$window','dialogs', function ($scope, $http, $state, $rootScope, $localStorage, sqlCypherDb, $interval,$location,$window,dialogs) {


    $scope.address = ["Server", "EHC1", "EHC2", "Other"];
    $scope.sAddress = $scope.address[0];
     
    $scope.addressSelectionChanged = function(choice)
    {
        $scope.sAddress = choice;
       
        if ($scope.sAddress == "Other") {
            $scope.opts = {
                'size': 'sm',
                'windowClass': 'align-modal-dialog'
            };
            var dialogInstance = dialogs.create('templates/dialogs/OtherDialog.html', 'modalOtherDialogController', {}, $scope.opts);
            dialogInstance.result.then(function (result) {
                $scope.createUrls(result.url);
            });


        }
        else {
            $scope.createUrls($rootScope.urlTemplate.servers[choice]);
    ...
    $scope.createUrls = function (choice) {
        
        $rootScope.urls = {};
        for (var i in $rootScope.urlTemplate.url) {
            $rootScope.urls[i] = choice + $rootScope.urlTemplate.url[i];
          //  console.log($rootScope.urls);
        }
    }
    $scope.addressSelectionChanged("Server");
...
app.controller('modalOtherDialogController', ['$scope', '$modalInstance', '$http', '$filter', 'data', '$rootScope', function ($scope, $modalInstance, $http, $filter, data, $rootScope) {
    ...
    $scope.save = function (ipAddress, portNum) {
        
        $scope.errorMessage1 = "";
        $scope.errorMessage2 = "";
        $scope.ipAddressClassColor1 = "";
        $scope.ipAddressClassColor2 = "";
        $scope.flagPort = 0;
        $scope.flagIp = 0;
        ...
        if (portNum >= 0 && portNum <= 65535) {
            ...
            $scope.errorMessage2 = "Invalid Port Number";
            $scope.ipAddressClassColor2 = { "border-color": "#ff0000", "border-width": "3%" };
        }
        var ipformat = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
        if (ipformat.exec(ipAddress)) {
            $scope.flagIp = 1;
        }
        else {
            $scope.errorMessage1 = "Invalid IP Address";
            $scope.ipAddressClassColor1 = { "border-color": "#ff0000", "border-width": "3%" };

        }

        if($scope.flagIp==1 && $scope.flagPort==1)
        {
    ...
    $modalInstance.close({ "url": $scope.details.address + ":" + $scope.details.port });
...
app.controller('patientSearchCtrl', function ($scope, $http, $location, $filter, $state, Idle, Keepalive, $rootScope, sqlCypherDb, $localStorage, urlHitService, dialogs, $stateParams, windowsFileDownload, $q, pdfSaver) {
    ...
    $scope.deletePatientData = function () {
        ...
        if (localStorage.getItem("ngStorage-lastDbSync")) {
            ...
            sqlCypherDb.deleteAllValuesFromTable("users").then(function (r) {
                $scope.dialogInstance = dialogs.create('templates/dialogs/deleteStatusDialog.html', 'loadingGifController', { message: "Patient Data deleted" },
         {
             'size': 'sm',
             'windowClass': 'align-modal-dialog'
         });
            ...
            $scope.dialogInstance = dialogs.create('templates/dialogs/deleteStatusDialog.html', 'loadingGifController', { message: "No Local Patient Data" },
        {
            'size': 'sm',
            'windowClass': 'align-modal-dialog'
        });
    ...
    //takes the logout url specified in urlsources.json
    $scope.logoutClicked = function () {                                     //function called on clicking logout button
        $http.get($scope.logoutUrl).                                 
           success(function (data, status, headers, config) {
    ...
    $scope.$on('IdleTimeout', function () {                                    //the app logs out when specified idle time expires
        $scope.logoutUrl = $rootScope.urls.logoutUrl;
        $http.get($scope.logoutUrl).
            success(function (data, status, headers, config) {
                console.log("logged Out");
...
app.controller('selectPatientCtrl', ['$scope', '$http', '$element', '$rootScope', '$state', '$localStorage', 'dialogs', '$stateParams', 'sqlCypherDb', '$filter', function ($scope, $http, $element, $rootScope, $state, $localStorage, dialogs, $stateParams, sqlCypherDb, $filter) {
    ...
    $scope.filterType = $rootScope.patientSearchState.filterType;               //taking the default value of filter type from rootscope i.e "id"
    $scope.patientSearchIdStyleClass = { "background-color": "#2A4455" };
    $scope.patientSearchNameStyleClass = { "background-color": "#4D687B" };
    $scope.patientSearchGenderStyleClass = { "background-color": "#4D687B" };     //default css color for tabs when page is loaded
    $scope.patientSearchDOBStyleClass = { "background-color": "#4D687B" };
    $scope.selectedHeader = { 'path': "templates/directives/patientSearchPage/filterType-textBox.html" };//default template shown when page is loaded
    ...
    $scope.patient = angular.copy($rootScope.patientSearchState.patient);
    ...
    $scope.onFilterTypeClicked = function (filter) {                      //defining a function being called  on selecting a particular tab with the argument of the tab type passed from the view
        
        $rootScope.$broadcast('patientSearchPage.filterTypeChanged');
        $scope.patientSearchIdStyleClass = { "background-color": "#4D687B" };
        $scope.patientSearchNameStyleClass = { "background-color": "#4D687B" };              //default css background color for the tabs on patient search page
        $scope.patientSearchGenderStyleClass = { "background-color": "#4D687B" };
        $scope.patientSearchDOBStyleClass = { "background-color": "#4D687B" };

        if (filter == "id") {                                            
            $scope.patientSearchIdStyleClass = { "background-color": "#2A4455" };                 //set  the background color of selected tab 
            $scope.textTypeSpec = "number";                                                      
            $scope.filterName = "Filter patients by patientID (Enter a number): ";                  //placeholder text for the text box in the loaded template
            if ($scope.filterType != "id") {
                $scope.patients = [];
            }
            $scope.filterType = "id";
            $scope.selectedHeader.path = "templates/directives/patientSearchPage/filterType-textBox.html"; //loads the template based on the tab selected
    ...
    $scope.onSearchClicked = function () {
        ...
        {                                                             //body to be executed when the search button is clicked for online login
            $scope.searchDesabled = true;                              //disable the search button after click
            $scope.patients = [];
            $scope.getPatientsList();                                  //calling a function to load the data from the server i.e. the list of patients
        }
    }
    ...
    $scope.getPatientsList = function () {
        ...
        if ($scope.filterType == "id") {
            ...
            console.log($scope.patient.filterText);
        }
        else if ($scope.filterType == "name") {
            $scope.urlType = $rootScope.urls.patientNameSearchUrl + $scope.patient.filterText + "/0/20";
        }
        ...
        $scope.patientSearchSuccessfull = function (data, status, headers, config) {
            ...
            if ($scope.filterType == "id") {
                ...
                console.log("data to be loaded "+$scope.dataToBeLoaded);
                $scope.setNumber = 1;
            ...
            console.log("data loaded "+$scope.dataLoaded);
...
app.controller('datePickerController', ['$scope', '$filter', 'dialogs','$rootScope', function ($scope, $filter, dialogs, $rootScope) {
    ...
    $scope.addPatientDates = {
      //  'dob': new Date().toLocaleDateString().replace("/","-").replace("/","-")
       // 'stats': new Date(),
        //'misc': new Date(),
       // 'iEfsDate': new Date(),
       // 'iDOB': new Date(),
    ...
    $scope.onNewDateSelected = function () {
        ...
        $scope.$parent.selectedDate = $scope.selctedDate;
    }

    $scope.$on("datePicker.newDateSelected", function (event, args) {
                
        $scope.addPatientDates[args[1]] = args[0];
...
app.controller('searchBarCtrl', ['$scope', '$filter', 'dialogs', function ($scope, $filter, dialogs) {
    $scope.patientFilterText = "";                                   //model variable set to empty(contains value entered in text box)
    $scope.filterTextChanged = function () {
        ...
        if ($scope.patientFilterText != "")
            ...
            switch ($scope.$parent.filterType) {
                ...
                case "name":
                    if ($scope.patientFilterText.slice(-1).match(/[A-Za-z]*/)[0] == "") {
...
app.controller('patientDetailsCtrl', function ($scope, $http, $location, $state, Idle, Keepalive, $stateParams, $sce, $localStorage, dialogs, $rootScope) {
    ...
    $scope.patientDataLoadedSuccessFunctionInit = function (data, status, headers, config) {
        if (data["patient"]) {
            ...
            console.log($rootScope.patientDetailsToBeDisplayed.patient);
        ...
        //    //}
        //    var a = $rootScope.patientDetailsToBeDisplayed["notes"].length;
        //    console.log(a);
        //    //for (var i = 0; i < data.notes.length; i++) {
        //    //    $scope.notesDemo.push({
        //    //        noteDate: data.notes[i].noteDate,
        //    //        noteTitle: data.notes[i].noteTitle,
        //    //        noteBody: data.notes[i].noteBody,
        //    //        noteAssignedTo:data.notes[i].noteAssignedTo
        //    //        //name: data.searchResult.patients[i].firstName + " " + data.searchResult.patients[i].lastName,
        //    //        //gender: data.searchResult.patients[i].gender,
        //    //        //dateOfBirth: data.searchResult.patients[i].dob,
        //    //        //patientID: data.searchResult.patients[i].id
        //    //    });
        //    //    console.log($scope.notesDemo);
        //    //}
        ...
        //            $scope.notes.push({
        //                noteDate: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed["notes"][i].noteDate),
        //                noteTitle: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed["notes"][i].noteTitle),
        //                noteBody: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed["notes"][i].noteBody),
        //                noteAssignedTo: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed["notes"][i].noteAssignedTo)
        //            });
        //            console.log($scope.notes);
        //            console.log("notes.length " + $scope.notes.length);
        ...
        //    else {
        //        $scope.noNotesMessage = "There are no notes on file for this patient.";
        ...
        console.log($scope.gotPatientData + " " + $scope.gotContactData + " ");
        ...
        if ( $scope.gotPatientData == true && $scope.gotContactData == true && $rootScope.patientDetailsToBeDisplayed.appoinments && $rootScope.patientDetailsToBeDisplayed.medicalHistory.allergies) {
        $scope.loaderGifClass.display = "none";
        }
    }
    ...
    if ($rootScope.patientDetailsToBeDisplayed["patient"].id && $rootScope.patientDetailsToBeDisplayed["contact"].patientID  && $rootScope.patientDetailsToBeDisplayed.appoinments && $rootScope.patientDetailsToBeDisplayed.medicalHistory.allergies) {
    $scope.loaderGifClass.display = "none";
    }
    //--------------------------------------
    ...
    $scope.onPatientDetailsTabClicked = function (info) {
        ...
        if (info == "patientInfo") {
            ...
            $scope.homeSelectedHeader.path = "templates/directives/patientDetailsPage/homeTabContents/notesTabSelectionPage.html";
...
app.controller('historyTabSelectionCtrl', ['$scope', '$http', '$filter', '$localStorage', 'dialogs', '$rootScope', function ($scope, $http, $filter, $localStorage, dialogs, $rootScope) { //controller attached to template
...
app.controller('selectAppointmentCtrl', ['$scope', '$filter', '$state', '$stateParams', '$http', '$rootScope', '$localStorage', 'dialogs', 'sqlCypherDb', function ($scope, $filter, $state, $stateParams, $http, $rootScope, $localStorage, dialogs, sqlCypherDb) {
    $scope.buttonTitle = "UPDATE LIST";                         //property of the scope for the view
    $scope.onAppointmentsDataLoaded = function (data, status, headers, config) {       //function passed to success function,called on successful ajax call
        ...
        $scope.filterAppointments.sort(function (a, b) {
            
                return parseFloat((a.startTime.split(':')[0]) + "" + (a.startTime.split(':')[1])) - parseFloat((b.startTime.split(':')[0]) + "" + (b.startTime.split(':')[1]));
    ...
    $scope.onUpdateButtonClicked = function () {                                   //function called on clicking the button created in view  with a click event associated with the function to be called
        $scope.$parent.loaderGifClass.display = "block";                            //spinner appears
        $http.get($rootScope.urls.patientAppointmentOnDateUrl + $filter('date')($scope.selectedDate, 'MM-dd-yyyy'))  //$http service with a function "get" to send ajax calls to server
        .success($scope.onAppointmentsDataLoaded)              //http.get() function returns a promise object with a success and error function
        .error($scope.onAppointmentsDataError);                 //if ajax call succeeeds the function passed to success function is executed else the function passed to error function is executed
    }
    $scope.filterAppointments = $rootScope.patientSearchState.appointmentsData;//$stateParams.patientListData.dataApointments;

    $scope.providerType = ["All Providers", "Administrator", "nurse nurse", "walt white"];   //array to populate the drop down 
    $scope.providerFilter = $scope.providerType[0];                                          //default value for drop down 
    $scope.providerSelectionChanged = function (choice) {                                     //function called when we select a value from a drop down
...
app.controller('addPatientCtrl', ['$scope', '$http', '$filter', '$rootScope', 'dialogs', '$localStorage', function ($scope, $http, $filter, $rootScope, dialogs, $localStorage) {
    ...
    console.log($rootScope.patientSearchState.addPatientData.countries);
    console.log($rootScope.patientSearchState.addPatientData.states);
    ...
    $scope.onSaveButtonClicked = function () {
        ...
        $scope.dataTobeSent = {
            "who":
            {
                ...
                "dob": $filter('date')($scope.addPatient.addPatientDates.dob, 'yyyy-MM-dd'),
        ...
        $http.post($rootScope.urls.patientUploadUrl, $scope.dataTobeSent)          //ajax call to post data to server with the url and data to be sent to server
...
app.controller('transactionsTabSelectionCtrl', ['$scope', '$http', '$rootScope', '$localStorage', function ($scope, $http, $rootScope, $localStorage) {
    $scope.onTransactionsDataLoaded = function (data, status, headers, config) {
        ...
        $scope.patientTransactions.sort(function (a, b) {
...
app.controller('disclosureTabSelectionCtrl', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
    $scope.onDisclosuresDataLoaded = function (data, status, headers, config) {           //function called when the disclosure data is loaded successfully
        $scope.patientDisclosure = [];
        //creates an empty array to store data retrieved on 

        $scope.$parent.loaderGifClass.display = "none";                          //spinner  disappears when data is loaded        
        for (var i = 0; i < data["Disclosures"].length; i++) {                   //looping through the data from server
            $scope.patientDisclosure.push({                                        //push the data from the server in the empty array
                "recepientName": data["Disclosures"][i].recipient,                      //push the data from server  in model variable which is displayed in the view
        ...
        $scope.patientDisclosure.sort(function (a, b) {
           
            var disclosureDate1 = a.description.split(" ")[0];
            var disclosureTime1 = a.description.split(" ")[1];
            var disclosureDate2 = b.description.split(" ")[0];
            var disclosureTime2 = b.description.split(" ")[1];
            var disc1 = disclosureDate1 + " " + disclosureTime1;
            var disc2 = disclosureDate2 + " " + disclosureTime2;
            console.log(disc1);
            console.log(disc2);
            
            return new Date(disc1).valueOf() - new Date(disc2).valueOf();
    ...
    if (!$rootScope.patientDetailsToBeDisplayed.disclosures) {                //executes if the disclosures default object in app.js is null
        $scope.$parent.loaderGifClass.display = "block";
        $scope.url = $rootScope.urls.patientDisclosureUrl + $rootScope.patientDetailsToBeDisplayed.patientID + "/" + "1"  //url to fetch disclosure data in urlsources.json
        $http.get($scope.url)                             //ajax call to get data from server with the url specified
            .success($scope.onDisclosuresDataLoaded)          //on successful ajax call
            .error($scope.onDisclosureDataError);             //executes when the call is not completed successfully
    }
    else {
...
app.controller('encountersTabSelectionCtrl', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
       ...
       $scope.encounterFilter = "";    //EDITED HERE
       
    $scope.onEncounterDataLoaded = function (data, status, headers, config) {
        console.log("in onEncounterDataLoaded");
        $scope.$parent.loaderGifClass.display = "none";
        if (data.encounters.mEncounters.length > 0) {
            ...
            if (!$scope.encounters[$scope.encounterFilter.encounterID].Vitals && !$scope.encounters[$scope.encounterFilter.encounterID]['Work/School Note'] &&
            !$scope.encounters[$scope.encounterFilter.encounterID]['Speech Dictation'] && !$scope.encounters[$scope.encounterFilter.encounterID].Soap) {
                $scope.noEncounters = "No Encounters to show";
    ...
    $scope.onEncountersDataLoaded = function (data, status, headers, config) {
        ...
        $scope.encounterHistoryType = [];


        if ($rootScope.patientDetailsToBeDisplayed.encounterData.length > 0) {
           
            for (var i = 0; i < $rootScope.patientDetailsToBeDisplayed.encounterData.length; i++) {

                $scope.encounterHistoryType.push({
                    id: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed.encounterData[i].id),
                    date: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed.encounterData[i].date),
                    visitCategory: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed.encounterData[i].visitCategory),
                    display: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed.encounterData[i].display),
                    encounterID: $scope.checkStringVal($rootScope.patientDetailsToBeDisplayed.encounterData[i].encounterID)
        ...
        $scope.encounterHistoryType.sort(function (a, b) {

            var encounterHistoryTypeDate1 = a.date.split("T")[0];
            var encounterHistoryTypeTime1 = a.date.split("T")[1];
            // console.log(disclosureDate1);
            var encounterHistoryTypeDate2 = b.date.split("T")[0];
            var encounterHistoryTypeTime2 = b.date.split("T")[1];
            //   console.log(disclosureDate2);
            var encounterData1 = encounterHistoryTypeDate1 + " " + encounterHistoryTypeTime1;
            var encounterData2 = encounterHistoryTypeDate2 + " " + encounterHistoryTypeTime2;
            console.log(new Date(encounterData1).valueOf());
           
            return new Date(encounterData2).valueOf() - new Date(encounterData1).valueOf();
        ...
        $rootScope.patientDetailsToBeDisplayed.encounterData = $scope.encounterHistoryType;
    ...
    $scope.encounterHistoryChanged = function (value) {
        ...
        console.log($scope.encounterFilter);
        if (!$scope.encounters[$scope.encounterFilter.encounterID]) {// if the encounter data is not previously fetched,only then call is made to the server
            $scope.$parent.loaderGifClass.display = "block";
...
app.controller('notesPageSelectionCtrl', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
    ...
    $scope.gotNotesData = false;
    $scope.noNotesMessage = "";
  
    $scope.onNotesDataLoaded = function (data, status, headers, config) {
        $scope.$parent.loaderGifClass.display = "none";
        if (data["notes"]) {
            $rootScope.patientDetailsToBeDisplayed.notes = data["notes"];
            $scope.gotNotesData = true;
            ...
            $scope.notesArrayLength = $rootScope.patientDetailsToBeDisplayed["notes"].length;
            ...
            if ($rootScope.patientDetailsToBeDisplayed["notes"].length > 0) {
                ...
                $rootScope.patientDetailsToBeDisplayed["notes"] = $scope.notes;
            }
    ...
    $scope.onNotesDataError = function (data, status, headers, config) {
        $scope.urlRefesh = config.url;
        if (status == 401) {
    ...
    if ($rootScope.patientDetailsToBeDisplayed["notes"].length == 0) {
        $scope.$parent.loaderGifClass.display = "block";
        $http.get($rootScope.urls.patientNotesUrl + $rootScope.patientDetailsToBeDisplayed.patientID).
               success($scope.onNotesDataLoaded).
               error($scope.onNotesDataError);
    }
    else {
        $scope.notesArrayLength = $rootScope.patientDetailsToBeDisplayed["notes"].length;
        $scope.notes = $rootScope.patientDetailsToBeDisplayed["notes"];
    }
...
/*app.controller('remindersPageSelectionCtrl', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
    $scope.onPatientRemindersTabClicked = function (reminder) {                  
        $scope.patientClinicalRemindersStyleClass = { "border-bottom": "#E4E8EB", "border-bottom-style": "solid" }; //default css color property to all the tabs when template is loaded
        $scope.patientRemindersStyleClass = { "border-bottom": "#E4E8EB", "border-bottom-style": "solid" };
        $scope.patientMessagesRemindersStyleClass = { "border-bottom": "#E4E8EB", "border-bottom-style": "solid" };
        ...
        if (reminder == "clinical") {
            ...
            $rootScope.patientDetailsToBeDisplayed.piTabState = 2;                          //maintaining the state of tab
            $scope.patientMessagesRemindersStyleClass = { "border-bottom": "#30B69D", "border-bottom-style": "solid" };   //change the color of selected tab using ng class specified in view
            $scope.remindersSelectedHeader.path = "templates/directives/patientDetailsPage/remindersTabContents/messagesTabContent.html";  //template specified when the particular tab under reminders is clicked
        }
    }
    $scope.clinicalReminders = {                      //object created with key value pairs so the view takes the data from the value specified
